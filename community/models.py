...
class Trade(models.Model):
    FOREXPAIRS_CHOICES = (
        ...
        ("NZD_USD" , "NZD_USD"), ("USD_CAD" , "USD_CAD"), ("USD_CHF" , "USD_CHF"), ("EUR_GBP" , "EUR_GBP"),
        ("GBP_JPY" , "GBP_JPY"), ("AUD_GBP" , "AUD_GBP"), ("GBP_NZD" , "GBP_NZD"), ("GBP_CAD" , "GBP_CAD"),
        ("GBP_CHF" , "GBP_CHF"), ("EUR_JPY" , "EUR_JPY"), ("EUR_AUD" , "EUR_AUD"), ("EUR_NZD" , "EUR_NZD"),
        ("EUR_CAD" , "EUR_CAD"), ("EUR_CHF" , "EUR_CHF"), ("AUD_JPY" , "AUD_JPY"), ("AUD_NZD" , "AUD_NZD"),
        ("AUD_CAD" , "AUD_CAD"), ("AUD_CHF" , "AUD_CHF"), ("CAD_JPY" , "CAD_JPY"), ("NZD_CAD" , "NZD_CAD"),
        ("CAD_CHF" , "CAD_CHF"), ("NZD_JPY" , "NZD_JPY"), ("NZD_CHF" , "NZD_CHF"), ("CHF_JPY" , "CHF_JPY"),
        ("XAU_USD" , "XAU_USD"), ("XBT_USD" , "XBT_USD"), ("XNG_USD" , "XNG_USD"), ("XTI_USD" , "XTI_USD"),
        ("BTC_USD" , "BTC_USD"),
    )
    WL_CHOICES = (
        ('WIN', 'WIN'), ('LOSE', 'LOSE'), ('BE', 'BE'),
    )
    CURRENCY_CHOICES = (
    ...
    def compressImage(self,image):
        ...
        if img.size[0] > 520:
            ...
            imgResize = img.resize((basewidth, hsize))
            if image.name.split('.')[1] == 'jpeg' or image.name.split('.')[1] == 'jpg':
                imgResize.save(im_io , format='jpeg', optimize=True, quality=55)
                new_image = File(im_io, name="%s.jpeg" % image.name.split('.')[0],)
            else:
                imgResize.save(im_io , format='png', optimize=True, quality=55)
...
class UserProfile(models.Model):
    ...
    def compressImageMain(self,image):
        if self.image:
            ...
            def save(self, *args, **kwargs):
                if not self.id:
                    self.image = self.compressImageT(self.image)
                super(UserProfile, self).save(*args, **kwargs)

            def compressImageT(self,image):
                img = Image.open(image).convert("RGB")
                im_io = BytesIO()
